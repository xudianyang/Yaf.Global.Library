...
class Operation implements IteratorAggregate
{
    ...
    public function setMetadata($key, $value, $overwriteArray = false)
    {
        ...
        if (!isset($_SESSION['__META'])) {
            $_SESSION['__META'] = array();
        }
        if (isset($_SESSION['__META'][$key]) && is_array($value)) {
            if ($overwriteArray) {
                $_SESSION['__META'][$key] = $value;
            } else {
                $_SESSION['__META'][$key] = array_replace_recursive($_SESSION['__META'][$key], $value);
            }
        } else {
            if ((null === $value) && isset($_SESSION['__META'][$key])) {
                $array = $_SESSION['__META'];
                unset($array[$key]);
                $_SESSION['__META'] = $array;
                unset($array);
            } elseif (null !== $value) {
    ...
    public function getMetadata($key = null)
    {
        ...
        if (null === $key) {
            return $_SESSION['__META'];
        }

        if (!array_key_exists($key, $_SESSION['__META'])) {
            return false;
        }

        return $_SESSION['__META'][$key];
    }
